...
public class ClientController {
        ...
        if (settings.getGrantType().equals("clientCredentials")) {
            ...
            String authorizationUrlComplete = String.format(
                    settings.getAuthorizationURL()
                            .concat("?response_type=%s&client_id=%s&scope=%s&state=example"), URLEncoder.encode(responseType, Charset.defaultCharset().name()), settings.getOauthKey(), encodedScopes);
            ...
            if ("form_post".equals(settings.getResponseMode())) {
                authorizationUrlComplete = authorizationUrlComplete + "&response_mode=form_post";
            }
    ...
    private String headersFromRequest(HttpServletRequest request) {
        ...
        if (settings.getGrantType().equals("implicit")) {
            ...
            if ("form_post".equals(settings.getResponseMode())) {
                if (parameterMap.containsKey("access_token")) {
                    ...
                    settings.setAccessTokenJson(parseJWT(accessToken));
                }
                if (parameterMap.containsKey("id_token")) {
    ...
    private String toView(ClientSettings settings) {
        return settings.isOpenIdConnect() ? "oidc-client" : "oauth-client";
    }
    ...
    private ClientSettings createDefaultSettings(String modus) {
